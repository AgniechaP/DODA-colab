...
ref_img_path = '/content/drive/MyDrive/DODA/v2/datasets/uavvaste_vol2_resized/Terraref_x9_vol3/'

output_path = '/content/drive/MyDrive/DODA/v2/output/Rubbish_test_after_bbox_size_change/'
    ...
    for n, out_img in enumerate(out_imgs):
        ...
        if bboxes != 'no_box':
            for box in bboxes:
                ...
                # ctr_image = cv2.rectangle(out_img, (box[0], box[1]), (box[0] + box[2], box[1] + box[3]), (208, 146, 0), 2)
                ctr_image = cv2.rectangle(out_img, (int(box[0]), int(box[1])), (int(box[2])+int(box[0]), int(box[3])+int(box[1])), (208, 146,0), 2)
